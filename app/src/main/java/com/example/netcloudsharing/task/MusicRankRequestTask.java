...
public class MusicRankRequestTask extends AsyncTask<String, Void, String> {
    private MusicRankRequestListener musicRankRequestListener;

    public MusicRankRequestTask(MusicRankRequestListener musicRankRequestListener) {
        this.musicRankRequestListener = musicRankRequestListener;
    }

    private static final String TAG = "MusicRankRequestTask";
...
//            Log.d(TAG, "requestMusicListHtml: " + responseData);
            ...
            musicRankRequestListener.onSearchComplete(HtmlParser.parseHtmlForMusicRank(responseData));
        } else {
            Log.e(TAG, "Failed to fetch music list HTML");
        }
    }
    public interface MusicRankRequestListener {
        void onSearchComplete(List<RankSetBean> beanList);
        void onSearchError();
    }
}
